 ...
 * has its own dedicated template-wiki-article.php: no Infobox sidebar,
 * category badge comes from native WordPress Categories instead of
 * the Content Type taxonomy, and the footer shows Tags plus a Related
...
while ( have_posts() ) :
	...
	<main id="main-content" class="lunar-article">
		<article <?php post_class( 'lunar-article__entry' ); ?> id="post-<?php the_ID(); ?>">
			...
			<?php
			// Silently omitted if this post has no category or no other
			// post shares one — there is nothing useful to relate it to.
