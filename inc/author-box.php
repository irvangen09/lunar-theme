 ...
 *
 * Role and social links come from Lunar Wiki's public API. If those
 * functions aren't available for any reason (e.g. the plugin is
 * inactive), those two pieces are simply omitted — name, avatar, and
 * bio (all native WordPress) still render on their own.
 *
...
function lunar_render_author_box( int $user_id = 0, bool $is_archive_heading = false ): void {
	...
	if ( function_exists( 'lunar_wiki_get_author_role' ) ) {
		$role = lunar_wiki_get_author_role( $user_id );
	}

	if ( function_exists( 'lunar_wiki_get_author_social_links' ) ) {
		$links = lunar_wiki_get_author_social_links( $user_id );
	}
