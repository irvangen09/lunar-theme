<?php
/**
 * Breadcrumb helper. Handles the single Wiki Article, native Post, Game
 * archive, and Author archive contexts. Search and Pages are
 * intentionally not handled here — lunar_breadcrumb() is a no-op for
 * both, by design, not because they're pending.
 *
 * @package Lunar
 */
...
function lunar_breadcrumb(): void {
	if ( function_exists( 'lunar_wiki_get_post_type_slug' ) && is_singular( lunar_wiki_get_post_type_slug() ) ) {
		$crumbs = lunar_get_breadcrumb_for_wiki_article();
	} elseif ( is_singular( 'post' ) ) {
		$crumbs = lunar_get_breadcrumb_for_post();
	} elseif ( function_exists( 'lunar_wiki_get_taxonomy_slug_game' ) && is_tax( lunar_wiki_get_taxonomy_slug_game() ) ) {
 ...
 * Home > Franchise > Specific Title > Content Type > Article Title.
 *
...
function lunar_get_breadcrumb_for_wiki_article(): array {
	...
	if ( $game_term ) {
		if ( 0 !== (int) $game_term->parent && function_exists( 'lunar_wiki_get_taxonomy_slug_game' ) ) {
			$franchise = get_term( $game_term->parent, lunar_wiki_get_taxonomy_slug_game() );
...
/**
 * Builds the breadcrumb trail for a native Post: Home > Category > Post Title.
 ...
 * - Franchise-level term: Home > Franchise
 * - Specific Title term:  Home > Franchise > Specific Title
 *
 * @return array<int, array{label: string, url: string}>
 */
function lunar_get_breadcrumb_for_game_archive(): array {
	...
	if ( function_exists( 'lunar_wiki_get_taxonomy_slug_game' ) ) {
		$franchise = get_term( $term->parent, lunar_wiki_get_taxonomy_slug_game() );
...
/**
 * Builds the breadcrumb trail for an Author archive: Home > Author Name.
 *
 * @return array<int, array{label: string, url: string}>
 */
