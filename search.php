<?php
/**
 * Search Result template — search box, Game checkbox filter, Content
 * Type pill filter, field-sync filters, results list, and pagination.
 *
 * @package Lunar
 */
...
$lunar_game_taxonomy  = function_exists( 'lunar_wiki_get_taxonomy_slug_game' ) ? lunar_wiki_get_taxonomy_slug_game() : 'game';
...
// Second argument must be omitted (or `false`) to fall back to the
// current URL — passing `null` explicitly makes this return '' instead.
	...
	<?php if ( have_posts() ) : ?>
		...
		<div class="lunar-result-list">
			...
			while ( have_posts() ) :
				...
				$lunar_game_post_terms         = get_the_terms( get_the_ID(), $lunar_game_taxonomy );
